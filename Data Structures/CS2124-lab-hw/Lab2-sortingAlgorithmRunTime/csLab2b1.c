...
int main( int argc, char *argv[] )
{
    ...
    unsigned sizes1[] = { 1000000000, 2500000000};
    int sizes2[] = { 10000, 20000, 40000};


    srand(time(0));

    // fRT = timeAlgorithm("Insertion Sort", 1, 3, sizes1, insertionSortInitial );
    // printRuntimeTable(fRT);
    // freeFunctionRuntimes(fRT);

    fRT = timeAlgorithm("Quick Sort", 2, 2, sizes1, quickSortOptInitial );
    printRuntimeTable(fRT);
    freeFunctionRuntimes(fRT);
...
void printRuntimeTable( functionRuntimes fRT )
{
    ...
    printf("Test Size: ");
    int testNum=0;
    for (int x = 0; x < fRT.numTestCases; x++)
    {
