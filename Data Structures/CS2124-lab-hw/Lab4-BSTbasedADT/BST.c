...
nodeT* deleteNode(nodeT *pRoot, int value){
    ...
    if (pRoot == NULL) {
        printf("-----error--------\n");
        printf("No found this item!\n");
        printf("The number not in the tree!\n");
        return pRoot;
    }
...
void preOrder(nodeT *p){
    ...
    printf("%d ", p->value);
    preOrder(p->pLeft);  //process nodes in left sub tree
    preOrder(p->pRight); //process nodes in right sub tree
}
